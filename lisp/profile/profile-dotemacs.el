...
(defun profile-dotemacs-percentage-color (percent col-begin col-end)
  ...
  (let* ((col1 (profile-dotemacs-color-values col-begin))
         (col2 (profile-dotemacs-color-values col-end))
         (col
          (mapcar (lambda (c)
                    (round
...
;; thumphries hack around unspecified-bg woes
(defun profile-dotemacs-color-values (face)
  (if (string= face "unspecified-bg") '(0 0 0) (color-values face)))
